...
#   python3 patch_game.py <build.html> <assets_dir>[,<assets_dir2>...] [out.html]
# each <assets_dir> holds one <id>_asset.json per model (from bake.py). Re-runnable: an earlier patch (including the
...
# one hook per family builder: (builder anchor line, hook appended right after it)
FAMILY_HOOKS = [
    (anchor, HOOK),
    ("function makeRatmanMesh(scale,opts){ opts=opts||{}; const s=scale||1; const v=opts.variant||'scout';",
     " if(!opts.primitive){ const m3=makeRatmanModel3D(v,s); if(m3) return m3; }   // Imyala's sculpted ratmen — roles without a sculpt (burrower) fall through to the primitive rig"),
]
...
for header in ("// ============================================================ GOBLIN SCOUT — the sculpted 3D model (Imyala)",
    ...
    if i >= 0:
        ...
        html = html[:i] + html[j:]
html = html.replace(OLD_HOOK, '')
for a_, h_ in FAMILY_HOOKS: html = html.replace(h_, '')
...
for a_, h_ in FAMILY_HOOKS:
    assert html.count(a_) == 1, ('family anchor count', a_[:40], html.count(a_))
    html = html.replace(a_, a_ + h_)
...
# ---- data blocks ----
blocks = []
for path in sorted(sum((glob.glob(os.path.join(d_, '*_asset.json')) for d_ in adir.split(',')), [])):
